...
void video_open(void *cls,int width,int height,const void *buffer, int buflen, int payloadtype, double timestamp)
{
 ...
 //   _videoName = [NSString stringWithFormat:@"PixlRec-%04d.mp4",rand()];
...
#if SAVE_264_ENABLE
    {
        NSString *fileName264 = [Utilities documentsPath:[NSString stringWithFormat:@"PixlRec-%04d.264",rand()]];
...
- (void)startRecordingScreen
{
    ...
    // 60 is the Frames Per Second
    NSString *heightFile = [[self applicationDocumentsDirectory].path stringByAppendingPathComponent:@"heightFile.txt"];
    NSString *heightCustom = [NSString stringWithContentsOfFile:heightFile encoding:NSUTF8StringEncoding error:nil];
    int heightReal = [heightCustom intValue];
    NSString *widthFile = [[self applicationDocumentsDirectory].path stringByAppendingPathComponent:@"widthFile.txt"];
    NSString *widthCustom = [NSString stringWithContentsOfFile:widthFile encoding:NSUTF8StringEncoding error:nil];
    int widthReal = [widthCustom intValue];
    NSString *fpsFile = [[self applicationDocumentsDirectory].path stringByAppendingPathComponent:@"fpsFile.txt"];
    NSString *fpsCustom = [NSString stringWithContentsOfFile:fpsFile encoding:NSUTF8StringEncoding error:nil];
    int fpsReal = [fpsCustom intValue];
    int ret = XinDawn_StartMediaServer("PixlRec",widthReal, heightReal, fpsReal, 47000,7101,"000000000", &ao);
    printf("=====ret=%d========\n",ret);
...
- (void)mergeAudio {
    ...
    NSString *orientationCustom = [NSString stringWithContentsOfFile:orientationFile encoding:NSUTF8StringEncoding error:nil];
    //NSString *prefs = @"180";
    degrees = [orientationCustom doubleValue];
    ...
    NSString *audioCustom = [NSString stringWithContentsOfFile:audioFile encoding:NSUTF8StringEncoding error:nil];
    
   if ([[NSFileManager defaultManager] fileExistsAtPath:audioPath] && [audioCustom  isEqual: @"yes"])
    {
